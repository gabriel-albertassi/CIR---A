...
export async function GET(req: NextRequest) {
  try {
    ...
    // PROTEÇÃO ANTI-LOOP (Desativada temporariamente a pedido do usuário)
    ...
    const createLabelTable = (
      exams: { name: string; key: string; dest: string }[],
      pName: string,
      hOrigin: string
    ) => {
      const labelBorder = {
      ...
      const cleanRegistro = (prof.registro || "")
        .replace("REGISTRO", "")
        .trim();

      const cleanCargo = (prof.cargo || "")
        .replace("CARGO", "")
        .trim();

      const profLine =
        `${prof.name.toUpperCase()}` +
        `${cleanRegistro ? ` – ${cleanRegistro.toUpperCase()}` : ""}` +
        `${cleanCargo ? ` – ${cleanCargo.toUpperCase()}` : ""}`;
